...
<article>
    ...
    <section class="pt-20 pb-24">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-end">
            <div class="lg:col-span-8">
                <span
                    class="block text-sm font-bold text-[var(--accent)] uppercase tracking-[0.2em] mb-8"><?= $page->subtitle()->or('Elite UX Case Study') ?></span>
    ...
    <!-- Stats Grid (Legacy or Block-based) -->
    <?php if ($page->modules()->isEmpty() && $page->stats()->isNotEmpty()): ?>
        <section class="mb-20">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-0 border-y border-white/10">
                <?php foreach ($page->stats()->toStructure() as $stat): ?>
                    <div
                        ...
                        <span
                            class="block text-sm font-bold text-[var(--accent)] uppercase tracking-[0.2em] mb-4"><?= $stat->label() ?></span>
    ...
    <?php if ($page->modules()->isNotEmpty()): ?>
        <section class="mb-32">
            <?php foreach ($page->modules()->toLayouts() as $layout): ?>
                <div class="grid grid-cols-1 md:grid-cols-<?= $layout->columns()->count() ?> gap-12 mb-24"
                    id="<?= $layout->id() ?>">
    ...
    <!-- Alternative Paths (Rejected Concepts) - Keep as separate section or allow moving to blocks? -->
                ...
                <?php foreach ($page->alternative_paths()->toStructure() as $concept): ?>
                    <div class="p-10 border border-white/10 hover:border-[var(--accent)]/30 transition-colors">
                        <div class="flex items-center gap-3 mb-6">
                            ...
                            <h4 class="uppercase tracking-[0.2em] text-[10px] font-bold text-white/60">
                                <?= $concept->concept() ?></h4>
                        </div>
    ...
    <?php if ($page->modules()->isEmpty()): ?>
            ...
            <section class="mb-32">
                <div class="aspect-[21/7] w-full bg-[var(--charcoal)] relative overflow-hidden group rounded-3xl">
                    <img alt="<?= $image->alt() ?>"
                        ...
                        src="<?= $image->resize(1200)->url() ?>" srcset="<?= $image->srcset([800, 1200, 1400]) ?>"
                        sizes="100vw" />
                    <div class="absolute inset-0 flex items-center justify-center bg-black/40">
                        <div class="text-center px-6">
                            <h3 class="serif-display text-5xl md:text-7xl text-white">Visualizing <span
                                    class="italic">ROI</span></h3>
                            <p class="text-[var(--accent)] tracking-[0.5em] uppercase text-xs mt-6 font-bold">Design that
        ...
        <?php if ($page->challenge_text()->isNotEmpty()): ?>
            <!-- Challenge Section -->
            <section class="mb-32">
                    ...
                    <div class="relative">
                        <?php if ($image = $page->challenge_image()->toFile()): ?>
                            <div class="aspect-[4/5] overflow-hidden bg-[var(--charcoal)] rounded-3xl">
                                <img alt="<?= $image->alt() ?>" class="w-full h-full object-cover grayscale"
                                    src="<?= $image->resize(800)->url() ?>" srcset="<?= $image->srcset([600, 800]) ?>"
                                    sizes="(min-width: 1024px) 50vw, 100vw" />
                            </div>
                            <p class="mt-6 text-[10px] uppercase tracking-widest text-[var(--text-muted)]"><?= $image->caption() ?>
                            </p>
                        <?php endif ?>
                    </div>
                    <div class="pt-10">
                            ...
                            <div class="space-y-10">
                                <?php foreach ($page->challenge_points()->toStructure() as $point): ?>
                                    <div class="flex items-start gap-6">
                                        <span
                                            class="material-symbols-outlined text-[var(--accent)] text-3xl"><?= $point->icon() ?></span>
                                        <div>
                                            <h4 class="font-bold uppercase tracking-widest text-xs mb-2 text-white">
                                                <?= $point->title() ?></h4>
        ...
        <?php if ($page->strategy_intro()->isNotEmpty()): ?>
            ...
            <section class="mb-32">
                <div class="max-w-4xl mb-24">
                    ...
                    <h3 class="serif-display text-5xl md:text-7xl mb-10">Engineering <span
                            class="italic text-[var(--accent)]">Elegance</span></h3>
        ...
        <section class="mb-40">
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
                <div class="lg:col-span-5">
                    ...
                    <h3 class="serif-display text-5xl mb-8">Accessibility <br /><span class="italic">&amp; Compliance</span>
                    </h3>
    ...
    <section class="mb-48 max-w-5xl mx-auto text-center">
        ...
        <?php if ($pdf = $page->case_study_pdf()->toFile()): ?>
            <a href="<?= $pdf->url() ?>" target="_blank"
                class="inline-flex items-center gap-6 py-6 px-12 border border-white/20 rounded-full hover:bg-[var(--accent)] hover:text-black hover:border-[var(--accent)] transition-all duration-300 cursor-pointer group">
