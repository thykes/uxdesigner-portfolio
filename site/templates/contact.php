    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24 items-start">
        <div class="lg:col-span-6">
            <div class="serif-display text-6xl md:text-7xl lg:text-8xl leading-[1.1] tracking-tight mb-16">
                <?= str_replace(['<p>', '</p>'], '', $page->headline()->kt()) ?>
            </div>
            <div class="space-y-12">
                <div>
                    <p class="text-xs uppercase tracking-[0.3em] text-[var(--text-muted)] mb-4">
                        <?= $page->direct_contact_title()->or('Direct Contact') ?>
                    </p>
                    <a class="serif-display text-2xl md:text-3xl contact-link-hover"
                        href="mailto:<?= $page->email()->or('hello@johndoe.design') ?>"><?= $page->email()->or('hello@johndoe.design') ?></a>
                </div>
                <div>
                    <p class="text-xs uppercase tracking-[0.3em] text-[var(--text-muted)] mb-6">
                        <?= $page->social_title()->or('Social Channels') ?>
                    </p>
                    <div class="flex gap-8">
                        ...
                        foreach ($site->social_links()->toStructure() as $social):
                            ...
                            ?>
                            <a class="text-[var(--text-main)] hover:text-[var(--accent)] transition-colors"
            ...
            <?php else: ?>
                ...
                <form class="space-y-12" method="POST" action="<?= $page->url() ?>">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
                        <div class="group">
                            <label
                                class="text-[10px] uppercase tracking-widest text-[var(--text-muted)] block mb-2">Name</label>
                            <input class="w-full text-lg font-light text-white placeholder:text-white/20"
                                placeholder="Johnathan Doe" type="text" name="name" value="<?= esc($data['name'] ?? '') ?>"
                                required />
                        </div>
                        <div class="group">
                            <label class="text-[10px] uppercase tracking-widest text-[var(--text-muted)] block mb-2">Email
                                Address</label>
                            <input class="w-full text-lg font-light text-white placeholder:text-white/20"
                    ...
                    <div class="group">
                        <label class="text-[10px] uppercase tracking-widest text-[var(--text-muted)] block mb-2">Project
                            Type</label>
                        <select class="w-full text-lg font-light text-white appearance-none cursor-pointer"
                            name="project_type">
                            ...
                            <option class="bg-[var(--bg-deep)]" <?= (isset($data['project_type']) && $data['project_type'] == 'Product Photography') ? 'selected' : '' ?>>Product Photography
                            </option>
                    ...
                    <div class="group">
                        <label
                            class="text-[10px] uppercase tracking-widest text-[var(--text-muted)] block mb-2">Message</label>
                        <textarea class="w-full text-lg font-light text-white placeholder:text-white/20 resize-none"
                            placeholder="Tell me about your vision..." rows="4" name="text"
                    ...
                    <div class="pt-6">
                        <button
                            class="w-full md:w-auto px-16 py-5 bg-[var(--accent)] text-black text-sm uppercase tracking-[0.2em] font-bold hover:bg-white transition-all duration-500"
